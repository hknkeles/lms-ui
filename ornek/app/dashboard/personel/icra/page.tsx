...
export default function IcraPersonelPage() {
  ...
  // Query - Sadece icra personelini getir
  const { data: allEmployees = [], isLoading } = useQuery({
    queryKey: ['employees'],
    queryFn: loadEmployeesFromStorage,
  })

  // İcra personelini filtrele
  const icraEmployees = useMemo(() => {
    return allEmployees.filter(emp => 
      emp.birim && emp.birim.toLowerCase().includes('icra') ||
      emp.birim && emp.birim.toLowerCase().includes('iflas') ||
      emp.unvan && emp.unvan.toLowerCase().includes('icra müdürü') ||
      emp.unvan && emp.unvan.toLowerCase().includes('icra müdür yardımcısı') ||
      emp.unvan && emp.unvan.toLowerCase().includes('icra memuru')
    )
  }, [allEmployees])

  // Filtered and sorted data
  const filteredData = useMemo(() => {
    let filtered = icraEmployees
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
        <span className="text-foreground font-medium flex items-center space-x-1 px-2 py-1 rounded-md bg-gradient-to-r from-purple-500/10 to-purple-500/5 text-purple-600 border border-purple-500/20 shadow-sm">
          <FileText className="h-4 w-4" />
          <span>İcra Personeli</span>
        </span>
      </motion.nav>
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            İcra ve iflas müdürlüklerinde çalışan personeli görüntüleyin
          </p>
        </div>
        <motion.button
      ...
      >
        ...
        <div className="px-4 py-4 border-t bg-muted/30">
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Toplam {filteredData.length} icra personeli
            </div>
