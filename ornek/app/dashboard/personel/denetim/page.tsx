...
export default function DenetimPersonelPage() {
  ...
  // Query - Sadece denetim personelini getir
  const { data: allEmployees = [], isLoading } = useQuery({
    queryKey: ['employees'],
    queryFn: loadEmployeesFromStorage,
  })

  // Denetim personelini filtrele
  const denetimEmployees = useMemo(() => {
    return allEmployees.filter(emp => 
      emp.birim && emp.birim.toLowerCase().includes('denetim') ||
      emp.birim && emp.birim.toLowerCase().includes('teftiş') ||
      emp.unvan && emp.unvan.toLowerCase().includes('denetçi') ||
      emp.unvan && emp.unvan.toLowerCase().includes('teftiş') ||
      emp.unvan && emp.unvan.toLowerCase().includes('müfettiş')
    )
  }, [allEmployees])

  // Filtered and sorted data
  const filteredData = useMemo(() => {
    let filtered = denetimEmployees
    ...
    return filtered
  }, [denetimEmployees, filters, sortBy])
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
        <span className="text-foreground font-medium flex items-center space-x-1 px-2 py-1 rounded-md bg-gradient-to-r from-orange-500/10 to-orange-500/5 text-orange-600 border border-orange-500/20 shadow-sm">
          <Eye className="h-4 w-4" />
          <span>Denetim Personeli</span>
        </span>
      </motion.nav>
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Denetim ve teftiş birimlerinde çalışan personeli görüntüleyin
          </p>
        </div>
        <motion.button
      ...
      >
        ...
        <div className="px-4 py-4 border-t bg-muted/30">
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Toplam {filteredData.length} denetim personeli
            </div>
