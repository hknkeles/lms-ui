...
const columns: ColumnDef<Employee>[] = [
  ...
  {
    accessorKey: 'temporaryDutyLocation',
    header: 'Geçici Görev Yeri',
    cell: ({ row }) => (
      <span className="text-sm text-foreground">
        {row.original.temporaryDutyLocation || 'Belirtilmemiş'}
      </span>
    ),
    size: 180,
  },
  {
    accessorKey: 'temporaryDutyEndDate',
    header: 'Görev Bitiş Tarihi',
    cell: ({ row }) => (
      <span className="text-sm text-foreground">
        {row.original.temporaryDutyEndDate 
          ? new Date(row.original.temporaryDutyEndDate).toLocaleDateString('tr-TR')
          : 'Belirtilmemiş'
        }
      </span>
    ),
...
export default function GeciciPersonelPage() {
  ...
  // Query - Sadece geçici görevdeki personeli getir
  const { data: allEmployees = [], isLoading } = useQuery({
    queryKey: ['employees'],
    queryFn: loadEmployeesFromStorage,
  })

  // Geçici görevdeki personeli filtrele
  const geciciEmployees = useMemo(() => {
    return allEmployees.filter(emp => 
      emp.status === 'temporary_duty' ||
      emp.temporaryDutyLocation ||
      emp.temporaryDutyEndDate
    )
  }, [allEmployees])

  // Filtered and sorted data
  const filteredData = useMemo(() => {
    let filtered = geciciEmployees
    ...
    return filtered
  }, [geciciEmployees, filters, sortBy])
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
        <span className="text-foreground font-medium flex items-center space-x-1 px-2 py-1 rounded-md bg-gradient-to-r from-yellow-500/10 to-yellow-500/5 text-yellow-600 border border-yellow-500/20 shadow-sm">
          <Clock className="h-4 w-4" />
          <span>Geçici Görevlendirilen Personel</span>
        </span>
      </motion.nav>
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            Geçici Görevlendirilen Personel
          </h1>
          <p className="text-muted-foreground mt-2">
            Geçici görevde bulunan personeli görüntüleyin
          </p>
        </div>
        <motion.button
      ...
      >
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr>
                  ...
                  >
                    Geçici görevde personel bulunamadı
                  </td>
                </tr>
              ) : (
                filteredData.map((employee, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className="text-sm text-foreground">
                        {employee.temporaryDutyLocation || 'Belirtilmemiş'}
                      </span>
                    </td>
        ...
        <div className="px-4 py-4 border-t bg-muted/30">
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Toplam {filteredData.length} geçici görevdeki personel
            </div>
