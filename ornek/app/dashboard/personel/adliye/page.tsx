...
export default function AdliyePersonelPage() {
  ...
  // Query - Sadece adliye personelini getir
  const { data: allEmployees = [], isLoading } = useQuery({
    queryKey: ['employees'],
    queryFn: loadEmployeesFromStorage,
  })

  // Adliye personelini filtrele
  const adliyeEmployees = useMemo(() => {
    return allEmployees.filter(emp => 
      emp.birim && emp.birim.toLowerCase().includes('adliye') ||
      emp.unvan && emp.unvan.toLowerCase().includes('hakim') ||
      emp.unvan && emp.unvan.toLowerCase().includes('savcı') ||
      emp.unvan && emp.unvan.toLowerCase().includes('avukat')
    )
  }, [allEmployees])

  // Filtered and sorted data
  const filteredData = useMemo(() => {
    let filtered = adliyeEmployees
    ...
    return filtered
  }, [adliyeEmployees, filters, sortBy])
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
        <span className="text-foreground font-medium flex items-center space-x-1 px-2 py-1 rounded-md bg-gradient-to-r from-blue-500/10 to-blue-500/5 text-blue-600 border border-blue-500/20 shadow-sm">
          <Scale className="h-4 w-4" />
          <span>Adliye Personeli</span>
        </span>
      </motion.nav>
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Adliye birimlerinde çalışan personeli görüntüleyin
          </p>
        </div>
        <motion.button
      ...
      >
        ...
        <div className="px-4 py-4 border-t bg-muted/30">
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Toplam {filteredData.length} adliye personeli
            </div>
